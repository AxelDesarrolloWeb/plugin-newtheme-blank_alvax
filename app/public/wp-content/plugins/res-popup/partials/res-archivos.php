...
function enqueue_style($hook)
{
    ...
    // Encolando la librería de bootstrap 5
    wp_enqueue_style(
        'bootstrap-css',
...
function enqueue_scripts($hook)
{
    ...
    wp_localize_script(
        ...
        array(
            ...
            'pageUrl' => admin_url('admin.php?page=res_popup')
        )
    );
...
function res_data_popup()
{
    ...
    if (current_user_can('manage_options')) {
        ...
        if ($tipo === 'add') {
            ...
            $lista = get_option('res_popup', array());
            if (!is_array($lista)) {
                $lista = array();
            }
            ...
            if (get_option('res_popup') === null) {
                $ok = add_option('res_popup', $lista, true);
            } else {
                $ok = update_option('res_popup', $lista, true);
            }
